...
int main(int argc, char *argv[])
{
  ...
  {
    server_addr.sin_addr.s_addr = inet_addr(cliEntry.masterHost.c_str());
    server_addr.sin_port = htons(cliEntry.masterPort);

    std::cerr << "\n"
              << connect(server_fd, (struct sockaddr *)&server_addr,
                         sizeof(server_addr));
    cliEntry.master_fd = server_fd;
    char hsBuffer[1024] = {0};

    // Handshake part 1
    std::string ping{"*1\r\n$4\r\nping\r\n"};

    send(server_fd, ping.c_str(), ping.size(), 0);
    int recv_msg = recv(server_fd, hsBuffer, sizeof(hsBuffer), 0);
    ...
    send(server_fd, res_1.c_str(), res_1.size(), 0);
    recv_msg = recv(server_fd, hsBuffer, sizeof(hsBuffer), 0);

    send(server_fd, res_2.c_str(), res_2.size(), 0);
    recv_msg = recv(server_fd, hsBuffer, sizeof(hsBuffer), 0);
    ...
    send(server_fd, res_psync.c_str(), res_psync.size(), 0);
    recv_msg = recv(server_fd, hsBuffer, sizeof(hsBuffer), 0);
