...
void Server::request_handler(int client_fd)
{
  ...
  while (true)
  {
    ...
    ParsedCommand pc = RespParser.parse_msg(buffer);
    std::cout << "Command: " << pc.command << std::endl;
    CommandHandler RespHandler(this->config, this->isReplica);
    std::string return_msg = RespHandler.process_commands(pc);
    ...
    if (pc.command == "psync" && !isReplica)
    {
      add_replica(client_fd, "localhost", this->config.hostPort);
    }

    if (!isReplica && replicas.size() > 0 && RespHandler.is_write(pc.command))
    {
...
bool Server::connect_master()
{
  ...
  if (inet_pton(AF_INET, this->config.masterHost.c_str(), &master_addr.sin_addr) <= 0)
  {
    std::cerr << "Invalid address/ Address not supported\n";
    return false;
  }

  if (connect(master_fd, (struct sockaddr *)&master_addr,
...
void Server::add_replica(int fd, const std::string &host, int port)
{
  ...
  replicas[fd] = Replica(fd, host, port);
}
